...
async def profile_dialog(update, context):
    ...
    if dialog.count == 1:
        ...
        my_message = await send_text(update, context, "_ChatGPT 🧠 генерирует ваш профиль..._")
...
async def opener_dialog(update, context):
    ...
    if dialog.count == 1:
        ...
        dialog.user["occupation"] = text
        await send_text(update, context, "Какая цель знакомства?")
    elif dialog.count == 5:
        dialog.user["goals"] = text
...
app = ApplicationBuilder().token("7917724878:AAGDAWIHCsev4kU8-40vI_027Tx5wmUZCpc").build()
